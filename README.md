...
Load: The main.py script loads your specified pipeline_config.json.

Initialize: The Orchestrator reads the config, identifies the start_agent, and prepares the initial_input.

Execute & Update: It executes the current agent. The agent's outputs are saved to a central pipeline_state dictionary, keyed by agent_id.output_name.

Route: The Orchestrator checks the routing rules to determine the next agent.

Repeat: Steps 3 and 4 are repeated until an agent has no next step.

Finalize: The Orchestrator uses the final_outputs mapping to present a clean, user-friendly result.

# 🚀 Getting Started
## Prerequisites

1. Python 3.8+
2. Ollama: Install and run Ollama from ollama.com.
...
Generated bash
git clone https://github.com/your-username/agentic-pipeline-framework.git
